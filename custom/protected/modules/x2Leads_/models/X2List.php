...
class X2List extends X2Model {
    ...
    public function behaviors(){
        return array(
            ...
            'LinkableBehavior' => array(
                ...
                'baseRoute' => '/x2Leads/x2Leads/list',
                'autoCompleteSource' => '/x2Leads/x2Leads/getLists',
            ),
    ...
    public function attributeLabels(){
        return array(
            'id' => 'ID',
            'assignedTo' => Yii::t('x2Leads', 'Owner'),
            'name' => Yii::t('x2Leads', 'Name'),
            'description' => Yii::t('x2Leads', 'Description'),
            'type' => Yii::t('x2Leads', 'Type'),
            'logicType' => Yii::t('x2Leads', 'Logic Type'),
            'modelName' => Yii::t('x2Leads', 'Record Type'),
            'visibility' => Yii::t('x2Leads', 'Visibility'),
            'count' => Yii::t('x2Leads', 'Members'),
            'createDate' => Yii::t('x2Leads', 'Create Date'),
            'lastUpdated' => Yii::t('x2Leads', 'Last Updated'),
        );
    }
    ...
    public static function getComparisonList(){
        return array(
            '=' => Yii::t('x2Leads', 'equals'),
            '>' => Yii::t('x2Leads', 'greater than'),
            '<' => Yii::t('x2Leads', 'less than'),
            '<>' => Yii::t('x2Leads', 'not equal to'),
            'contains' => Yii::t('x2Leads', 'contains'),
            'noContains' => Yii::t('x2Leads', 'does not contain'),
            'empty' => Yii::t('x2Leads', 'empty'),
            'notEmpty' => Yii::t('x2Leads', 'not empty'),
            'list' => Yii::t('x2Leads', 'in list'),
            'notList' => Yii::t('x2Leads', 'not in list'),
        );
    }

    public function getDefaultRoute(){
        return '/x2Leads/x2Leads/list';
    }

    public function getDisplayName ($plural=true, $ofModule=true) {
        return Yii::t('x2Leads', '{contact} Lists|{contact} List', array(
            (int) $plural,
            '{contact}' => Modules::displayName(false, 'X2Leads'),
        ));
    }
    ...
    public function calculateCount () {
        ...
        return X2Leads::model ()->count ($criteria);
    }
    ...
    public function queryCriteria($useAccessRules = true){
        ...
        if($useAccessRules){
            $accessCriteria = X2Model::model('X2Leads')->getAccessCriteria(); // record-level access control for X2Leads
...
//        );
    ...
    public function campaignDataProvider($pageSize = null){
		$criteria = X2Model::model('X2Leads')->getAccessCriteria();
     ...
     * @param bool $allowNewsletter whether to allow x2Leads to be added to a newsletter
     */
    ...
    public static function getRoute($id){
        if($id == 'all')
            return array('/x2Leads/x2Leads/index');
        else if($id == 'new')
            return array('/x2Leads/x2Leads/newX2Leads');
        else if(empty($id) || $id == 'my')
            return array('/x2Leads/x2Leads/myX2Leads');
        else
            return array('/x2Leads/x2Leads/list', 'id' => $id);
    }
    ...
    public function setAttributes($values, $safeOnly = true){
        if($this->type == 'dynamic'){
            ...
            if(count($this->criteriaInput['attribute']) !== count($this->criteriaInput['value'])){
                $this->addError('', Yii::t('x2Leads','Invalid list criteria.'));
            }else{
                ...
                if (array_key_exists ('tags', $criteria) && empty($criteria['tags'])) {
                    $this->addError ('tags', Yii::t ('x2Leads', 'Tag list must be non-empty'));
                }
            }
        }
