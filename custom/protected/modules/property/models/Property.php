...
/**
 * This is the model class for table "x2_property".
 *
 * @package application.modules.property.models
 */
class Property extends X2Model {
    ...
    public static function getAvailableContacts($propertyId = 0) {

        $availableContacts = array();

        $criteria = new CDbCriteria;
        $criteria->addCondition("propertyId='$propertyId'");
        $criteria->addCondition(array("propertyId=''"), 'OR');
    ...
    public static function getContacts($propertyId) {
        $contacts = array();
        $contactRecords = CActiveRecord::model('Contacts')->findAllByAttributes(array('propertyId' => $propertyId));
    ...
    public static function setContacts($contactIds, $propertyId) {

        $account = CActiveRecord::model('Property')->findByPk($propertyId);

        if (!isset($account))
            return false;

        // get all contacts currently associated
        $oldContacts = CActiveRecord::model('Contacts')->findAllByAttributes(array('propertyId' => $propertyId));
        foreach ($oldContacts as $contact) {
            if (!in_array($contact->id, $contactIds)) {
                $contact->propertyId = 0;
        ...
        foreach ($contactIds as $id) {
            ...
            $contactRecord->propertyId = $account->id;
     ...
     * @return type
     */
    public function searchMyProperty() {
        $criteria = new CDbCriteria;
     ...
     * @return type
     */
    public function searchNewProperty() {
        $criteria = new CDbCriteria;
    ...
    public function searchList($id, $pageSize = null) {
        ...
        if (isset($list)) {
            ...
            return new SmartActiveDataProvider('Property', array(
                'criteria' => $search,
                'sort' => array(
