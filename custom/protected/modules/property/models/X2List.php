...
class X2List extends X2Model {
    ...
    public function behaviors(){
        return array(
            ...
            'LinkableBehavior' => array(
                ...
                'baseRoute' => '/property/property/list',
                'autoCompleteSource' => '/property/property/getLists',
            ),
    ...
    public function attributeLabels(){
        return array(
            'id' => 'ID',
            'assignedTo' => Yii::t('property', 'Owner'),
            'name' => Yii::t('property', 'Name'),
            'description' => Yii::t('accounts', 'Description'),
            'logicType' => Yii::t('property', 'Logic Type'),
            'modelName' => Yii::t('property', 'Record Type'),
            'visibility' => Yii::t('property', 'Visibility'),
            'count' => Yii::t('property', 'Members'),
            'createDate' => Yii::t('property', 'Create Date'),
            'lastUpdated' => Yii::t('property', 'Last Updated'),
        );
    }
    ...
    public static function getComparisonList(){
        return array(
            '=' => Yii::t('property', 'equals'),
            '>' => Yii::t('property', 'greater than'),
            '<' => Yii::t('property', 'less than'),
            '<>' => Yii::t('property', 'not equal to'),
            'contains' => Yii::t('property', 'contains'),
            'noContains' => Yii::t('property', 'does not contain'),
            'empty' => Yii::t('property', 'empty'),
            'notEmpty' => Yii::t('property', 'not empty'),
            'list' => Yii::t('property', 'in list'),
            'notList' => Yii::t('property', 'not in list'),
        );
    }
    ...
    public function getDisplayName ($plural=true, $ofModule=true) {
        return Yii::t('property', '{contact} Lists|{contact} List', array(
            (int) $plural,
            '{contact}' => Modules::displayName(false, 'Property'),
        ));
    }
    ...
    public function queryCriteria($useAccessRules = true){
        ...
        if($useAccessRules){
            $accessCriteria = X2Model::model('Property')->getAccessCriteria(); // record-level access control for X2Leads
...
//        );
     ...
     * @param bool $allowNewsletter whether to allow property to be added to a newsletter
     */
    ...
    public static function getRoute($id){
        if($id == 'all')
            return array('/property/property/index');
        else if($id == 'new')
            return array('/property/property/newProperty');
        else if(empty($id) || $id == 'my')
            return array('/property/property/myProperty');
        else
            return array('/property/property/list', 'id' => $id);
    }
    ...
    public function setAttributes($values, $safeOnly = true){
        if($this->type == 'dynamic'){
            ...
            if(count($this->criteriaInput['attribute']) !== count($this->criteriaInput['value'])){
                $this->addError('', Yii::t('property','Invalid list criteria.'));
            }else{
                ...
                if (array_key_exists ('tags', $criteria) && empty($criteria['tags'])) {
                    $this->addError ('tags', Yii::t ('property', 'Tag list must be non-empty'));
                }
            }
        }
