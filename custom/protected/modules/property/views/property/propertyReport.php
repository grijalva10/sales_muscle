...
$itemModel = new Property;
$criteriaArr = array();
if(!empty($_GET)){
    $_SESSION['propertyReport']['GET'] = json_encode($_GET, false);
}else{
    $_SESSION['propertyReport']['GET'] = '';
}
if(isset($_GET['Property'])){
    $filterAttributes = $_GET['Property'];
...
function createAttributeCell() {
	...
	div.attr('class', 'cell');
	var label = $('<label for=\"Property[attribute][]\">Attribute</label>');
	var dropdown = createDropdown(attributeLabels);
	dropdown.attr('name', 'Property[attribute][]').attr('onchange','updateForm(this);');
...
function createComparisonCell(field) {
	...
	var div = $(document.createElement('div'));
	div.attr('class', 'cell');
	var label = $('<label for=\"Property[comparison][]\">Comparison</label>');
	var dropdown = createDropdown(comparisonList, ignoreList);
	dropdown.attr('name', 'Property[comparison][]');
...
function createValueCell(field) {
	...
	switch(fieldTypes[field]) {
		...
		case 'visibility':
			...
			input.attr('name', 'Property[value][]');
		...
		case 'link':
			...
			hidden.attr('name', 'Property[value][]');
			input = $('<input size=\"30\" type=\"text\" value=\"\">');
            input.blur(function(){
	...
	var div = $(document.createElement('div'));
	div.attr('class', 'cell');
	var label = $('<label for=\"Property[value][]\">Value</label>');
...
?>
<div class="page-title"><h2><?php echo Yii::t('property', '{module} Report', array('{module}'=>Modules::displayName(true, 'Property'))); ?></h2></div>
<div class="form">
    <?php
    ...
    <div class="row">
        <div class="cell">
            <?php echo CHtml::label(Yii::t('property', 'Date Field'), 'dateField'); ?>
            <?php
        ...
        <div class="cell">
            <a href="#" class="x2-button" id="filters-link" style="margin-top:15px;"><?php echo Yii::t('property', 'Advanced Filters'); ?></a>
        </div>
        <div class="right">
            <a href="#" id="export-button" class="x2-button" style="margin-top:15px;"><?php echo Yii::t('property', 'Export Data'); ?></a>
        </div>
    </div>
    <div class="row">
...
<?php if(!empty($dataProvider)){ ?>
    <div id="account-campaign-form" style="display:none;" class="row form">
        <?php
        echo "<h3>".Yii::t('property','Campaign Form')."</h3>";
        echo Yii::t('property', 'This form will allow you to generate a campaign to mail related {contacts} of the {property} displayed in the report below. Select "Primary {contacts}" to only email {contacts} which are set as the Primary {contact} for an {account}. Select "All" to email all related {contacts} on each {account}.', array(
                '{account}'=>Modules::displayName (false ,'Property'),
                '{property}'=>Modules::displayName (true, 'Property'),
                '{contact}'=>Modules::displayName (false, 'Contacts'),
                '{contacts}'=>Modules::displayName (true, 'Contacts'),
            ));
        echo "<br><br>";
        echo CHtml::beginForm('propertyCampaign');
        echo CHtml::dropDownList('listType', '', array(
            'primary' => Yii::t('property', 'Primary {module}', array('{module}'=>Modules::displayName (true, 'Contacts'))),
            'all' => Yii::t('property', 'All {module}', array('{module}'=>Modules::displayName (true, 'Contacts'))),
        ));
...
<div id="report-data">
    ...
    if(!empty($dataProvider)){
        $this->widget('X2GridView', array(
            'id' => 'property-grid',
            'title' => Modules::displayName (true, 'Property'),
            'buttons' => array('columnSelector','autoResize'),
            'template' => '<div class="page-title icon property rounded-top">{title}{buttons}{summary}</div>{items}{pager}',
...
<div id="export-wrapper" class="form" style="display:none;">
    ...
    <div style="display:none" id="download-link-box">
        <?php echo Yii::t('property', 'Please click the link below to download this report.'); ?><br><br>
...
<script>
    ...
    $('#export-button').on('click',function(){
        exportPropertyReport(0);
    });
    function exportPropertyReport(page){
        if($('#export-status').length==0){
            $('#report-data').hide();
            $('#export-wrapper').show();
            $('#status-text').append("<div id='export-status'>Exporting <b><?php echo Modules::displayName(false, 'Property'); ?></b> data...<br></div>");
        }
        $.ajax({
            url:'exportPropertyReport?page='+page,
            success:function(data){
                if(data>0){
                    $('#export-status').html(((data)*100)+" records from <b><?php echo Modules::displayName(true, 'property'); ?></b> successfully exported.<br>");
                    exportPropertyReport(data);
                }else{
    ...
    $('#download-link').click(function(e) {
        ...
        window.location.href = '<?php echo $this->createUrl('/admin/downloadData', array('file' => $_SESSION['propertyReport']['propertyReportFile'])); ?>';
    });</script>
