...
$ConFields = X2Model::model("Property")->getFields();
foreach($ConFields as $field){
    echo '<input type="hidden" id="Property[' . $field->fieldName . ']" name="Property[' . $field->fieldName . ']">';      
}
...
$('#content').on('mouseup','#property-grid a',function(e) {
    document.cookie = 'vcr-list=".$listModel->id."; expires=0; path=/';
});

$('#createList').unbind('click').click(function() {
    var selectedItems = $.fn.yiiGridView.getChecked('property-grid','C_gvCheckbox');
    if(selectedItems.length > 0) {
        var listName = prompt('".addslashes(Yii::t('app','What should the list be named?'))."','');

        if(listName != '' && listName != null) {
            $.ajax({
                url:'".$this->createUrl('/property/property/createListFromSelection')."',
                type:'post',
                data:{listName:listName,modelName:'Property',gvSelection:selectedItems},
...
$('#addToList').unbind('click').click(function() {
    ...
    if(selectedItems.length > 0) {
        $.ajax({
            url:'".$this->createUrl('/property/property/addToList')."',
            type:'post',
...
$('#removeFromList').unbind('click').click(function() {
    ...
    if(selectedItems.length > 0) {
        ...
        if(confirmRemove) {
            $.ajax({
                url:'".$this->createUrl('/property/property/removeFromList')."',
                type:'post',
                data:{listId:".$listModel->id.",gvSelection:selectedItems},
                success:function(response) { if(response=='success') $.fn.yiiGridView.update('property-grid'); else alert(response); }
            });
        }
    }
    return false;
...
$this->widget('X2GridView', array(
    ...
    'template'=> 
        ...
        '<div id="x2-gridview-page-title" '.
         'class="page-title x2-gridview-fixed-title">{title}{buttons}{massActionButtons}'
            .(Yii::app()->user->checkAccess('AdminExportModels',array('module'=>'property')) ? 
                CHtml::link(
                    Yii::t('app','Export'),
                    array('/admin/exportModels','model'=>'Property', 'listId'=>$listModel->id),
    ...
    // 'columns'=>$columns,
    'modelName'=>'Property',
    'viewName'=>'property_list'.$listModel->id,
    // 'columnSelectorId'=>'property-column-selector',
